...
trait GroupsLike { this: S2StringContextCreation =>
  trait AutoNumberedGroup extends ExamplesGroup {
    ...
    def :=[R](a: Action[R])(implicit asResult: AsResult[R], p2: ImplicitParam2) {
      autoNumberedExamples = autoNumberedExamples :+ ExecutionVar.actionResult(a)(asResult, p2)
    }
...
object ExecutionVar {
  ...
  def actionResult[R](a: Action[R])(implicit asResult: AsResult[R], p2: ImplicitParam2) =
    new ExecutionVar(() => Execution.withEnvAsync(_ => control.futureAction(a)))
